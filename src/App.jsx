...
function App() {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            let history = await fetch(`https://www.lolytics.org/match_history?name=${name}&tag=${tag}&region=${region}`);
            history = await history.json();

            let matchDetails = [];
            for (const match_id of history.match_ids.slice(0, 5)) {
                matchDetails.push(fetch(`https://www.lolytics.org/match_details?id=${match_id}&puuid=${history.puuid}&region=${region}`));
            }
    ...
    const handleMatchClick = async (i) => {
        ...
        let response = await fetch(`https://www.lolytics.org/match_analysis?id=${matchImages[i].id}&puuid=${matchImages[i].puuid}&region=${region}`);
