...
export function process_champion_kill(state, event) {
	...
	if (event.killerId > 0) {
		state.teams[team_id].players[(event.killerId - 1) % 5].kill_gold += event.bounty + event.shutdownBounty;
	}
	...
	if (event.assistingParticipantIds) {
		...
		for (const assist_id of event.assistingParticipantIds) {
			...
			state.teams[assist_team_id].players[(assist_id - 1) % 5].assist_gold += (event.bounty + event.shutdownBounty) * assist_split;
		}
	}
}
